...
```{r basics 1}
#1.
# Using the sequence function to generate a sequence from 1 to 30 by 3.
sequence1 <- seq(1,30,3)
sequence1

#2. 
# Using the mean() function to determine the mean of sequence1. the function name is mean.
mean <- mean(sequence1)
mean

# Using the median() function to determine the median of sequence1. the function name is median.
...
#7.
#turned the series of vectors into a data frame.
students_df <- as.data.frame(students) 
students_df

#8.
# renamed the students_df columns.
...
```{r basics 3}
#10.
# `if` and `else` statement did not work, used the `ifelse` statement instead.
