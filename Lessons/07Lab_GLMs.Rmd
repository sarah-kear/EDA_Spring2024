...
plot1 <- EPAair %>%
  ...
  labs(fill = "Year") +
  ggtitle("Density Plot: Ozone by year") +
  mytheme
plot1
...
plot3 <- EPAair %>%
  ggplot(aes(x = as.factor(Year),
             fill = Ozone)) +
  geom_violin(draw_quantile = 0.50)  +
  #stat_summary(geom="text",fun=max,vjust=-1,size=4,label=c("b","a")) +
  mytheme
plot3
...
plot4 <- EPAair %>%
  ggplot(aes(x = Ozone,
             y = PM2.5)) +
  geom_point() +
  geom_smooth(method = "lm", col="blue") # HOW TO ADD LINE OF BEST FIT
  #stat_summary(geom="text",fun=max,vjust=-1,size=4,label=c("b","a")) +
  mytheme
plot4
...
All <- lm(data - EPAair.subset,
          PM2.5 ~ Ozone + as.factor(Month) + mean_Lat + mean_Long)
