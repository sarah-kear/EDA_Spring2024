...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # LIKE THE TEXT TO COLUMN IN EXCEL
  mutate( Date = my(paste0(Month,"-",Year))) %>% #USE PASTE0 IS THERE IS NO SEPARATOR
...
BR_complete <- inner_join(Rainfall_BR_processed,Temp_BR_processed) #will join based on same date, more useful than cbind because cbind just binds them together, this one determines based on same characteristics
...
plot1 # the intervals between each year is the same space so like same time --- indicates there is seasonality.
...
f_month <- month(first(BR_complete$Date)) # NEED THE FIRST OBSERVATION IS AT THE FIRST ROW
f_year <- year(first(BR_complete$Date))

br_rainfall_ts <- ts(BR_complete$Rainfall,
                   start=c(f_year,f_month), #this automates the first month instead of looking at the data
                   frequency=12)
...
temp_decomp <- stl(br_temp_ts, s.window = "periodic")
plot(temp_decomp)
...
rainfall_data <-as.data.frame(rainfall_decomp$time.series) %>%
  mutate(Observed=BR_complete$Rainfall,
         Date=BR_complete$Date)

temp_data <-as.data.frame(temp_decomp$time.series) %>%
  mutate(Observed=BR_complete$Rainfall,
         Date=BR_complete$Date)
...
# if there is seasonal component, use season mann-kendall test

test1<- trend::smk.test(br_rainfall_ts)
test1
summary(test1)
# conclude there is a trend
# the s tells you whether it is positive or negative. 7071 ie positive, positive trend

test2<- trend::smk.test(br_temp_ts)
test2
summary(test2)
# conclude there is a trend
# the s tells you whether it is positive or negative. 7071 ie positive, positive trend
